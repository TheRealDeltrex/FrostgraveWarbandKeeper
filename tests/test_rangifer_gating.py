...
def test_rangifer_blocked_without_the_book(fresh_warband):
    ...
    ok, msg = warband_store.add_soldier(wb, "rangifer", "Scout")
    assert not ok
    assert "rangifer" in msg.lower()


def test_rangifer_hireable_with_the_book_in_vault(fresh_warband):
    wb = fresh_warband
    _enable(wb)
    warband_store.add_vault_item(wb, "Book of the Rangifer")
    assert expansions.soldier_state_block(wb, "rangifer") is None
    ok, msg = warband_store.add_soldier(wb, "rangifer", "Scout")
    assert ok, msg
